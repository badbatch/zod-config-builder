...
Use the script below or its `build` equivalent to transform a config builder file into a file that default exports a literally typed object like the one in the following example. Use the `--experimental-strip-types` Node.js flag on older versions that do not support running Typescript natively.

```sh
NODE_OPTIONS="--experimental-strip-types" npx zcb watch ./configBuilder.ts ./builtConfig.ts
```
